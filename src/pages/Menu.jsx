...
const Menu = () => {
  ...
  const addToCart = (product, cantidad = 1) => {
    setCart((prevCart) => {
      ...
      if (existingProduct) {
        showToastAñadir(
          `Añadido ${cantidad} <span class="ToastCarrito-name">${product.name}</span> más al carrito`
        );
        ...
        showToastAñadir(
          `Añadido ${cantidad} <span class="ToastCarrito-name">${product.name}</span> al carrito`
        );
    ...
    try {
      const res = await fetch(`${API_URL}${API_ROUTER}${API_COMPRAS}`, {
        ...
        body: JSON.stringify({
          mesa: mesaId,
          sesionId: sesionId,
          productos: cart.map(({ _id, quantity }) => ({
            producto: _id,
            cantidad: quantity,
          })),
        }),
    ...
    <div className="PageWrap">
      {!isMobile && (
        <aside className="DetalleProducto">
          <img
            className="PageImg"
            src="/img/sushi-table.jpg"
            alt="Mesas del restaurante"
          />
      ...
      <main className="Menu">
        <MesaHeader
          ...
          setModalHistorialAbierto={setModalHistorialAbierto}
        />
        <LogoutConfirmModal
          isOpen={modalAbierto}
